...
app.get('/reviews?latitude=<some-lat>&longitude=<some-longitude>', async (req, res) => {
  try {
    const response = await (await request.get(`https://api.yelp.com/v3/businesses/search?latitude=${lat}&longitude=${lng}`)).set('Authorization', 'Bearer 88kbtsnNHkXGPry8YiL15JHHve1wCFSMhwQfizLJbuK3aemfz1TjC5jGvcb5WNqRWCTOcXMgh5UtPJz26iE14rvToH6amZxqS9XcSHugrNWnbCJKBSDi-_Fu1iObYHYx');
    const weather = formatWeather(response.body);
    res.json(weather);
  }
  catch(err) {
